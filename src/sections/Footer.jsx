  ...
  return (
    <footer className="max-container">
        ...
        <div className="flex flex-wrap justify-between flex-1 gap-20 lg:gap-10">
          {footerLinks.map((section) => (
            <div className="" key={section.title}>
              <h4 className="mb-6 text-2xl leading-normal text-white font-montserrat">
                {section.title}
              </h4>
              <ul>
                {section.links.map((link) => (
                  <li
                    className="mt-3 leading-normal cursor-pointer text-white-400 font-montserrat hover:text-slate-gray"
                    key={link.name}
                  >
      ...
      <div className="flex justify-between mt-24 text-white-400 max-sm:flex-col max-sm:items-center">
        ...
        <p className="cursor-pointer font-montserrat text-white-400">Terms and Conditions.</p>
      </div>
    </footer>
  );
};
